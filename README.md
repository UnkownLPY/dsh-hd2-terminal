...
- **实时 UPLINK 状态**：订阅浏览器与后端连接状态，断链时绿色「上行链路安全」变为红色「超级驱逐舰离开轨道」且指示灯停止闪烁，恢复后自动回到绿色
...
- 「设置 → 常规 → 超级地球终端」可随时开关主题、「主题语言」可切换中文/English，切换即时生效且持久化。
...
感谢 [daxianliu23-creator](https://github.com/daxianliu23-creator)（FSV830）提供的中文文案、CJK 字体回退与实时 UPLINK 状态补丁——本插件的双语界面基于其贡献实现（commit 含 Co-authored-by 署名）。
