	...
	factory: (require) => {
		...
		const TEXT = {
			zh: {
				...
				dismiss: '关闭',
				helldiverAlt: '绝地潜兵',
				panelStats: [['自由', '100%'], ['叛国', '0.0001%'], ['战争债券', '∞'], ['管理式民主', '在线'], ['自由协议', '激活中']],
				quotes: ['“不投身战斗的公民，是不尽职的公民。”', '“民主必须传播，必要时以武力。”'],
				truth: '真理部',
				federation: '超级地球联邦 2184',
				seActionTitle: '超级地球公民简报',
				sestDay: '白天',
				sestNight: '夜晚',
				textPatches: [['Session log', '获取战术简报'], ['Deep diving...', '请民主的等待']],
			},
			en: {
				propaganda: [
				...
				subtitle: 'DEMOCRACY TACTICAL TERMINAL',
				secure: 'UPLINK SECURE',
				secureDown: 'SUPER DESTROYER LEFT ORBIT',
				emblemAlt: 'Super Earth',
				infobtn: 'Citizen briefing',
				recon: 'RECON',
				rounds: 'ROUNDS',
				steps: 'STEPS',
				tools: 'TOOLS',
				...
				uplinkTag: '◤ UPLINK ◢',
				signal: 'SIGNAL 98%',
				panelTitle: 'CITIZEN BRIEFING',
				dismiss: 'DISMISS',
				helldiverAlt: 'Helldiver',
				panelStats: [['LIBERTY', '100%'], ['TREASON', '0.0001%'], ['WAR BONDS', '∞'], ['MANAGED DEMOCRACY', 'ONLINE'], ['FREEDOM PROTOCOL', 'ACTIVE']],
				quotes: ['“A CITIZEN WHO DOES NOT DIVE IS A CITIZEN WHO DOES NOT SERVE.”', '“DEMOCRACY MUST BE SPREAD. BY FORCE IF NECESSARY.”'],
				truth: 'MINISTRY OF TRUTH',
				federation: 'SUPER EARTH FEDERATION 2184',
				seActionTitle: 'Super Earth citizen briefing',
				sestDay: 'day',
				sestNight: 'night',
				textPatches: [],
			},
		...
		function subscribeLang(fn) { langListeners.add(fn); return () => { langListeners.delete(fn) } }
		function getLang() { return lang }
		function setLang(next) {
			const value = next === "en" ? "en" : "zh"
			if (value === lang) return
			lang = value
			langListeners.forEach((fn) => { try { fn() } catch (_e) {} })
		}
		function useLang() {
			const [l, setL] = React.useState(getLang())
			React.useEffect(() => subscribeLang(() => setL(getLang())), [])
			return l
		}

		function tickerText(l) {
			return TEXT[l].ticker.map(function (t) { return '★ ' + t + ' ' }).join('  ')
		}
		...
		function SestClock() {
			...
			const glyph = isDay
				? React.createElement('svg', { className: 'hd2-sest-icon', viewBox: '0 0 256 256', width: 9, height: 9, 'aria-label': TEXT[l].sestDay },
				...
				: React.createElement('span', { className: 'hd2-sest-moon', 'aria-label': TEXT[l].sestNight }, '☾')
		...
		function Propaganda() {
			...
			React.useEffect(() => {
				if (runtimeCtx === null) return undefined
				return runtimeCtx.interval(() => setI((v) => (v + 1) % TEXT[l].propaganda.length), 4200)
			}, [l])
			return React.createElement('span', { className: 'hd2-propaganda', key: l + ':' + i }, TEXT[l].propaganda[i])
		}
		...
		function InfoPanel() {
			const l = useLang()
			const stats = TEXT[l].panelStats
			return React.createElement('div', { className: 'hd2-panel', role: 'dialog' },
				React.createElement('div', { className: 'hd2-panel-head' },
					React.createElement('span', { className: 'hd2-panel-title' }, TEXT[l].panelTitle),
					React.createElement('button', { className: 'hd2-panel-close', onClick: toggleBriefing }, TEXT[l].dismiss),
				),
				React.createElement('div', { className: 'hd2-panel-body' },
					React.createElement('img', { src: HELLDIVER_DATA, alt: TEXT[l].helldiverAlt, className: 'hd2-panel-emblem', draggable: false }),
					...
					React.createElement('div', { className: 'hd2-quote' }, TEXT[l].quotes[0]),
					React.createElement('div', { className: 'hd2-quote' }, TEXT[l].quotes[1]),
					React.createElement('div', { className: 'hd2-footnote' }, TEXT[l].truth),
					React.createElement('div', { className: 'hd2-footnote' }, TEXT[l].federation),
				),
			)
		}

		function useUplinkDown() {
			const [down, setDown] = React.useState(false)
			React.useEffect(() => {
				try {
					const c = runtimeCtx === null ? null : runtimeCtx.get("connection")
					if (c === null || c === undefined || c.hostDescription === undefined) return undefined
					return c.hostDescription.subscribe(() => {
						setDown(c.hostDescription.getSnapshot() === undefined)
					})
			...
			return React.createElement('div', { className: 'hd2-frame' },
				React.createElement('div', { className: 'hd2-topbar' },
					React.createElement('div', { className: 'hd2-topbar-left' },
						React.createElement('img', { src: EMBLEM_DATA, alt: TEXT[l].emblemAlt, className: 'hd2-emblem', draggable: false }),
						React.createElement('div', { className: 'hd2-titles' },
							React.createElement('div', { className: 'hd2-title' }, TEXT[l].superEarth),
							React.createElement('div', { className: 'hd2-subtitle' }, TEXT[l].subtitle),
						),
					),
					...
					React.createElement('div', { className: 'hd2-topbar-right' },
						React.createElement('span', { className: 'hd2-secure' + (down ? ' hd2-secure-down' : '') }, down ? TEXT[l].secureDown : TEXT[l].secure),
						React.createElement(SestClock),
						React.createElement('button', { className: 'hd2-infobtn', title: TEXT[l].infobtn, onClick: toggleBriefing }, 'ⓘ'),
					),
				),
				React.createElement('div', { className: 'hd2-ticker' },
					React.createElement('div', { className: 'hd2-ticker-track' },
						React.createElement('span', { className: 'hd2-ticker-seg' }, tickerText(l)),
						React.createElement('span', { className: 'hd2-ticker-seg', 'aria-hidden': 'true' }, tickerText(l)),
					),
				),
		...
		function ReconReadout(props) {
			...
			return React.createElement('div', { className: 'hd2-recon' },
				React.createElement('span', { className: 'hd2-recon-title' }, TEXT[l].recon),
				React.createElement('span', { className: 'hd2-recon-value' }, TEXT[l].rounds + ' ' + rounds),
			)
		}
		...
		function ComposerStats(props) {
			...
			if (stats && typeof stats === 'object' && stats.steps > 0) {
				parts.push(TEXT[l].rounds + ' ' + stats.turns + ' · ' + TEXT[l].steps + ' ' + stats.steps)
				const dur = []
				if (stats.llmMs > 0) dur.push('LLM ' + formatDuration(stats.llmMs))
				if (stats.toolMs > 0) dur.push(TEXT[l].tools + ' ' + formatDuration(stats.toolMs))
			...
			if (usage && typeof usage === 'object') {
				...
				if (billed > 0 || out > 0) {
					if (billed > 0) parts.push(TEXT[l].cache + ' ' + Math.round((usage.cacheReadTokens || 0) / billed * 100) + '%')
					parts.push(TEXT[l].input + ' ' + formatTokens(billed) + ' · ' + TEXT[l].output + ' ' + formatTokens(out))
				}
			}
			const body = parts.join(' | ')
			return React.createElement('div', { className: 'hd2-stats', title: body },
				React.createElement('span', { className: 'hd2-stats-tag' }, TEXT[l].uplinkTag),
				...
				React.createElement('span', { className: 'hd2-stats-right' },
					...
					React.createElement('span', { className: 'hd2-stats-sig' }, TEXT[l].signal),
				),
			)
		}
		...
		function SuperEarthAction(props) {
			...
			return React.createElement('button', {
				...
				title: TEXT[l].seActionTitle,
				onClick: toggleBriefing,
			},
				React.createElement('span', { className: 'hd2-se-action-star' }, '★'),
				wide ? React.createElement('span', null, TEXT[l].superEarth) : null,
			)
		}
		...
		const zh = {
			"hd2set.title": "超级地球终端",
			"hd2set.hint": "《绝地潜兵2》民主战术终端主题（粉丝作品）",
			"hd2set.on": "开启",
			"hd2set.off": "关闭",
			"hd2lang.title": "主题语言",
			"hd2lang.zh": "中文",
		...
		function createLanguageStore() {
			return runtime_client.defineStore({
				init: () => ({ language: "zh", revision: -1 }),
				actions: { sync: (d, language, revision) => {
					if (revision <= d.revision) return
					d.language = language === "en" ? "en" : "zh"
					d.revision = revision
				} },
			})
		}

		function LanguageRow({ t, useStore, setLanguage }) {
			const language = useStore((s) => s.language)
			return React.createElement('div', { className: 'hd2lang-group' },
				React.createElement('div', { className: 'hd2lang-title' }, t("hd2lang.title")),
				React.createElement('div', { className: 'hd2lang-row' },
					React.createElement('button', {
						type: 'button',
						className: 'hd2lang-opt' + (language === "zh" ? ' hd2lang-on' : ''),
						'aria-pressed': language === "zh",
						onClick: () => setLanguage("zh"),
					}, t("hd2lang.zh")),
					React.createElement('button', {
						type: 'button',
						className: 'hd2lang-opt' + (language === "en" ? ' hd2lang-on' : ''),
		...
		/* ---- theme-side text patches (Simplified Chinese mode only) ---- */
		let textObserver = null
		let textPatchPending = false
		const patchedNodes = []
		function applyTextPatches() {
			...
			while ((node = walker.nextNode()) !== null) {
				...
				if (parent !== null && parent.closest("input, textarea, [contenteditable], [data-input-mirror], [data-input-backdrop]") !== null) continue
				for (const [from, to] of TEXT[getLang()].textPatches) {
					if (node.nodeValue === from) { targets.push([node, to]); break }
				}
			}
			for (const [node, to] of targets) {
				patchedNodes.push([node, node.nodeValue])
				node.nodeValue = to
			}
		}
		function restoreTextPatches() {
			for (const [node, original] of patchedNodes) node.nodeValue = original
			patchedNodes.length = 0
		}
		function stopTextPatches() {
		...
		function startTextPatches() {
			if (typeof document === "undefined" || textObserver !== null) return
			if (getLang() !== "zh") return
			applyTextPatches()
			textObserver = new MutationObserver(() => {
				if (textPatchPending) return
				textPatchPending = true
				requestAnimationFrame(() => {
					textPatchPending = false
					applyTextPatches()
				})
			})
			textObserver.observe(document.body, { childList: true, subtree: true, characterData: true })
		}
		function syncTextPatches() {
			if (getLang() === "zh") startTextPatches()
			else stopTextPatches()
		}

		/* ---- theme settings transport (plugin-owned host route) ----
		 * The api proxy only serves its own allowlisted settings namespaces,
		 * so the client reads/writes the theme's namespace through a private
		 * web route registered by the host half. The `settings/document-updated`
		 * forwarded event keeps this store converged across external edits. */
		const SETTINGS_ENDPOINT = "/hd2-terminal/settings"
		let settingsSnap = { section: undefined, revision: undefined }
		const settingsListeners = new Set()
		function publishSettings(next) {
			settingsSnap = next
			settingsListeners.forEach((fn) => { try { fn() } catch (_e) {} })
		}
		function getSettingsSnap() { return settingsSnap }
		function subscribeSettings(fn) { settingsListeners.add(fn); return () => { settingsListeners.delete(fn) } }
		async function loadSettings() {
			let next = { section: undefined, revision: undefined }
			try {
				const res = await fetch(SETTINGS_ENDPOINT, { cache: "no-store" })
				if (!res.ok) throw new Error("settings endpoint HTTP " + res.status)
				const data = await res.json()
				if (data && data.ok) next = { section: data.value, revision: data.revision }
				else throw new Error("settings endpoint refused")
			} catch (_e) {
				if (getSettingsSnap().section !== undefined) return
			}
		...
		async function writeSettings(patch) {
			try {
				const res = await fetch(SETTINGS_ENDPOINT, {
					...
					body: JSON.stringify({ ...patch, expectedRevision: getSettingsSnap().revision }),
				})
				const data = await res.json()
				if (res.ok && data && data.ok) publishSettings({ section: data.value, revision: data.revision })
				else await loadSettings()
			} catch (_e) { await loadSettings() }
		}
		...
		const GLOBAL_CSS = [
			...
			".hd2-secure-down { color: var(--dsw-alias-state-error-primary); }",
			".hd2-secure-down:before { background: currentColor !important; animation: none !important; }",
			...
			"}",
			".hd2lang-group { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 2px 0; }",
			".hd2lang-title { color: var(--dsw-alias-label-primary); font-size: 14px; line-height: 22px; }",
			".hd2lang-row { display: flex; gap: 6px; }",
			".hd2lang-opt { cursor: pointer; background: transparent; border: 1px solid var(--dsw-alias-border-l2); color: var(--dsw-alias-label-secondary); border-radius: 6px; padding: 4px 12px; font-size: 12px; line-height: 18px; font-family: inherit; }",
			".hd2lang-opt.hd2lang-on { color: var(--hd2-yellow); border-color: var(--hd2-yellow); background: rgba(255, 231, 16, 0.08); }",
			".hd2-frame, .hd2-topbar, .hd2-ticker, .hd2-panel, .hd2-stats, .hd2-recon, .hd2-se-action, .hd2set-group, .hd2lang-group { font-family: 'Share Tech Mono', 'Chakra Petch', 'PingFang SC', 'Microsoft YaHei', sans-serif; }",
			".hd2-title, .hd2-panel-title { font-family: 'Black Ops One', 'Chakra Petch', 'PingFang SC', 'Microsoft YaHei', sans-serif; }",
			".hd2-propaganda, .hd2-ticker-seg, .hd2-stat, .hd2-quote, .hd2-footnote, .hd2-secure, .hd2-sest, .hd2-panel-close { font-family: 'Share Tech Mono', 'PingFang SC', 'Microsoft YaHei', sans-serif; }",
			".hd2-propaganda, .hd2-ticker-seg { letter-spacing: 0.1em; }",
		].join('\n')

		const inject = ["slots", "locale", "connection", "remote", "timer"]
		...
		function apply(ctx) {
			...
			function mount() {
				...
				if (slots !== undefined) {
					...
					injectDisposers.push(slots.inject("sidebar.footer.action", () => slots.register(
						{ name: "sidebar.footer.action", id: "hd2-superearth", order: 30, label: () => TEXT[getLang()].superEarth },
			...
			const syncRow = () => {
				const current = getSettingsSnap()
				const section = current.section
				let on, nextLang
				if (section === undefined) {
					on = typeof window !== "undefined" && window.__HD2_TERMINAL_ENABLED__ !== false
					nextLang = typeof window !== "undefined" && window.__HD2_TERMINAL_LANGUAGE__ === "en" ? "en" : "zh"
				} else {
					on = section.enabled !== false
					nextLang = section.language === "en" ? "en" : "zh"
				}
				setLang(nextLang)
				syncTextPatches()
				if (bound !== null) bound.sync(on, current.revision)
				if (boundLang !== null) boundLang.sync(nextLang, current.revision)
				if (on) mount()
				else unmount()
			}
			const offSettings = subscribeSettings(syncRow)
			loadSettings().then(() => { syncRow() })
			syncRow()
			const remote = ctx.get("remote")
			let offRemote = null
			if (remote !== undefined && typeof remote.$on === "function") {
				offRemote = remote.$on("settings/document-updated", (ns) => {
					if (ns !== undefined && ns !== "hd2-terminal") return
					loadSettings()
				})
			}
			ctx.effect(() => {
